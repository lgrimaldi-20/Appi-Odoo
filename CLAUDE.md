...
Covered by [tests/test_odoo19.py](tests/test_odoo19.py). These tests assert the *protocol and call shape* (authenticate over login, the `inventory_quantity_set` flag, `=` over `in`, the `parent_state` filter) against mocks — they cannot prove behaviour against a live 19 instance, which is still pending verification.
