"""
Router de PAGOS: POST /pagos.

Recibe un registro de pago de la DB de origen, lo sincroniza con Odoo
(crea account.payment + action_post) de forma idempotente y devuelve el id_odoo.
"""
...
from core.tasks import sincronizar_pago_task
from odoo_universal import OdooConnectionError, OdooUniversalAPI

logger = logging.getLogger("api-odoo")

router = APIRouter(tags=["Pagos"], dependencies=[Depends(verify_api_key)])


class PagoRequest(BaseModel):
    """Registro de pago de la base de datos de origen."""
    registro: dict = Field(..., description="Datos crudos del pago de origen.")
    tenant: str = "default"
...
def sincronizar_pago(req: PagoRequest, request: Request):
    """
    Sincroniza un pago hacia Odoo (crea + postea).

    Idempotente: si el pago ya fue procesado, devuelve el id_odoo existente
    con `idempotente: true` y no vuelve a tocar Odoo.

    Con `"async": true` encola la tarea y responde 202 con un task_id; el
    resultado se consulta luego en GET /estado/pago/{id_origen}.
    """
    odoo: OdooUniversalAPI = resolver_tenant(req.tenant)

    if req.procesar_async:
        task = sincronizar_pago_task.delay(req.registro, req.tenant)
        return {"encolado": True, "task_id": task.id, "tenant": req.tenant}

    try:
        resultado = crear_pago(req.registro, odoo)
        ...
        logger.warning("PAGO_ERROR | %s", e)
        raise HTTPException(status_code=422, detail=str(e))
    except OdooConnectionError as e:
        logger.error("PAGO_CONEXION | %s", e)
